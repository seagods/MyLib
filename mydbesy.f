      SUBROUTINE MYDBESY (X, FNU, N, Y, IERR)
...
C
      EXTERNAL DYAIRY
      INTEGER I, IFLW, J, N, NB, ND, NN, NUD, NULIM,IERR
      INTEGER I1MACH
...
C     write(6,*)"left dbesynu call from dbesy"
C     write(6,*)"setting ierr to zero"
      IERR=0
      RETURN
  120 CONTINUE
      J = NUD
      ...
      J = J + 1
      Y(J) = DBESY0(X)
C     write(6,*)"left dbesy0 call from dbesy"
C     write(6,*)"setting ierr to zero"
      IERR=0
      IF (ND.EQ.1) RETURN
      J = J + 1
