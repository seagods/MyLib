...
int myGaussElim(int &dim, double** Mpp, double* rhsV, double* lhsV){
	...
	double divide;
	double largest;
        double largestinrow, multiply, factor;
        int ireturn;
	...
	for(int i=0; i<dim; i++){
		...
		largest=0.0; 
		     ...
		     for(int k=i; k<dim; k++){
			absval=fabs(Mpp[Whereis[k]][i])/scalevec[Whereis[k]];
			if(absval>largest){
			       largest=absval;
			       klarge=Whereis[k];
			}}
