...
C     COMMON BLOCK USED BY LA05 () PACKAGE..
      ...
      integer NPR014
      integer NPR015
C     This is a hacked version of the original slatec file
C     I have replaced all the ASSIGN and ASSIGNED GO TO statements
C
...
20013 IF (.NOT.(SINGLR)) GO TO 20014
     ...
     +   IOPT)
      INFO=-NERR
      GO TO 30001
C20014 ASSIGN 20018 TO NPR005
20014 NPR005=20018
      GO TO 30005
C20018 ASSIGN 20019 TO NPR006
20018 NPR006=20019
      GO TO 30006
C20019 ASSIGN 20020 TO NPR007
20019 NPR007=20020
      GO TO 30007
20020 IF (.NOT.(USRBAS)) GO TO 20021
C     ASSIGN 20024 TO NPR008
      NPR008=20024
      GO TO 30008
...
C     ASSIGN 20029 TO NPR009
      NPR009=20029
      GO TO 30009
C20029 ASSIGN 20030 TO NPR010
20029 NPR010=20030
      GO TO 30010
C20030 ASSIGN 20031 TO NPR006
20030 NPR006=20031
      GO TO 30006
C20031 ASSIGN 20032 TO NPR008
20031 NPR008=20032
      GO TO 30008
...
C     COSTSC) AND PERFORM STANDARD PHASE-1.
     ...
     *IDG)
      SCOSTS=COSTSC
      COSTSC=ZERO
C     ASSIGN 20036 TO NPR007
      NPR007=20036
      GO TO 30007
C20036 ASSIGN 20037 TO NPR009
20036 NPR009=20037
      GO TO 30009
C20037 ASSIGN 20038 TO NPR010
20037 NPR010=20038
      GO TO 30010
C20038 ASSIGN 20039 TO NPR006
20038 NPR006=20039
      GO TO 30006
C20039 ASSIGN 20040 TO NPR008
20039 NPR008=20040
      GO TO 30008
...
20048 GO TO 20035
20045 XLAMDA=ZERO
C     ASSIGN 20050 TO NPR009
      NPR009=20050
      GO TO 30009
20050 CONTINUE
20034 CONTINUE
C
C20035 ASSIGN 20051 TO NPR011
20035 NPR011=20051
      GO TO 30011
...
20123 GO TO 20119
20121 CONTINUE
C     GO TO NPR010, (20030,20038)
      if(NPR010.eq.20030)go to 20030
      if(NPR010.eq.20038)go to 20038
C     PROCEDURE (PERFORM SIMPLEX STEPS)
C30009 ASSIGN 20134 TO NPR013
30009 NPR013=20134
      GO TO 30013
C20134 ASSIGN 20135 TO NPR014
20134 NPR014=20135
      GO TO 30014
...
20155 GO TO 20139
20140 CONTINUE
C     GO TO NPR009, (20029,20037,20044,20050)
      if(NPR009.eq.20029)go to 20029
      if(NPR009.eq.20037)go to 20037
      if(NPR009.eq.20044)go to 20044
      if(NPR009.eq.20050)go to 20050
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
20165 UU=0.1
     ...
     *SINGLR,REDBAS)
      ...
      GO TO 30001
20168 CONTINUE
C     GO TO NPR004, (20013,20204,20242)
      if(NPR004.eq.20013)go to 20013
      if(NPR004.eq.20204)go to 20204
      if(NPR004.eq.20242)go to 20242
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
20176 GO TO 20172
20174 CONTINUE
C     GO TO NPR007, (20020,20036)
      if(NPR007.eq.20020)go to 20020
      if(NPR007.eq.20036)go to 20036
...
20197 NERR=26
     ...
     +   NERR, IOPT)
      ...
      GO TO 30001
20193 CONTINUE
C     GO TO NPR005, (20018,20154,20243)
      if(NPR005.eq.20018)go to 20018
      if(NPR005.eq.20154)go to 20154
      if(NPR005.eq.20243)go to 20243
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C     TRY AN ABSOLUTE ERROR TEST IF THE RELATIVE TEST FAILS.
      ...
      CALL SCOPY(MRELAS,PRIMAL(NVARS+1),0,PRIMAL(NVARS+1),1)
20227 CONTINUE
C     GO TO NPR008, (20024,20032,20040)
      if(NPR008.eq.20024)go to 20024
      if(NPR008.eq.20032)go to 20032
      if(NPR008.eq.20040)go to 20040
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
30014 CALL SPINCW(
     ...
     *STPEDG)
C
C     GO TO NPR014, (20135,20246)
      if(NPR014.eq.20135)go to 20135
      if(NPR014.eq.20246)go to 20246
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     PROCEDURE (CHECK AND RETURN WITH EXCESS ITERATIONS)
30019 IF (.NOT.(ITLP.GT.MXITLP)) GO TO 20230
      NERR=25
C     ASSIGN 20233 TO NPR011
      NPR011=20233 
      GO TO 30011
C++  CODE FOR OUTPUT=YES IS ACTIVE
20233 IF (.NOT.(KPRINT.GE.1)) GO TO 20234
C     ASSIGN 20237 TO NPR012
      NPR012=20237 
      GO TO 30012
20237 CONTINUE
20234 CONTINUE
...
C     ASSIGN 20242 TO NPR004
      NPR004=20242
      GO TO 30004
C20242 ASSIGN 20243 TO NPR005
20242 NPR005=20243
      GO TO 30005
C20243 ASSIGN 20244 TO NPR006
20243 NPR006=20244
      GO TO 30006
C20244 ASSIGN 20245 TO NPR013
20244 NPR013=20245
      GO TO 30013
C20245 ASSIGN 20246 TO NPR014
20245 NPR014=20246
      GO TO 30014
20246 CONTINUE
C
...
C     GO TO NPR006, (20019,20031,20039,20244,20275)
      if(NPR006.eq.20019)go to 20019
      if(NPR006.eq.20031)go to 20031
      if(NPR006.eq.20039)go to 20039
      if(NPR006.eq.20244)go to 20244
      if(NPR006.eq.20275)go to 20275
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C     GO TO NPR013, (20134,20245,20267)
      if(NPR013.eq.20134)go to 20134
      if(NPR013.eq.20245)go to 20245
      if(NPR013.eq.20267)go to 20267
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
30015 CALL SPLPFE(
     ...
     *FOUND)
C     GO TO NPR015, (20141,20251)
      if(NPR015.eq.20141)go to 20141
      if(NPR015.eq.20251)go to 20251
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
20294 DUALS(MRELAS+J)=RZJ
      GO TO 20290
20292 CONTINUE
C     GO TO NPR011, (20051,20233)
      if(NPR011.eq.20051)go to 20051
      if(NPR011.eq.20233)go to 20233
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C++  CODE FOR OUTPUT=YES IS ACTIVE
...
C     GO TO NPR012, (20096,20237)
      if(NPR012.eq.20096)go to 20096
      if(NPR012.eq.20237)go to 20237
C CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
